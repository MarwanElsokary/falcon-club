...
/// شاشة الكشاف الرئيسية — مستقلة تماماً عن ClubMainScreen
          ...
          ValueListenableBuilder<int>(
            ...
            builder: (context, index, _) {
              return IndexedStack(
                index: index,
                children: [
                  // 0 — الرئيسية (home خاص بالكشاف)
                  // 0 — الرئيسية
                  MultiBlocProvider(
                    providers: [
                      BlocProvider(
                        create: (_) => getIt<ExperimentsCubit>()..emitbestTrials(categoryId: ''),
                      ),
                      BlocProvider(
                        create: (_) => getIt<ScoutTrainingCubit>()..fetchExercises(categoryId: '', popular: true),
                      ),
                      BlocProvider(
                        create: (_) => getIt<MainCubit>()..emitMyProfile()..emitCategories(),
                      ),
                      BlocProvider(
                      ...
                      // ✅ ضيف ده
                      BlocProvider(
                        create: (_) => getIt<TrainingCubit>()..emitallExercises(categoryId: '', popular: true),
                      ),
                    ],
                    child: ScoutHomeScreen(onDrawerTap: _toggleDrawer),
                  ),

                  // 1 — التمارين (read-only للكشاف)
                  MultiBlocProvider(
                    providers: [
                  ...
                  // 3 — اللاعبين المفضلين (read-only للكشاف)
